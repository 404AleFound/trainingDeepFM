...
可以看到模型在第 32 个 Epoch 左右在测试集上的性能最佳，AUC 达到0.7835，验证集损失 LogLoss = 0.4650，训练集损失 logLoss = 0.4643，而在这之后开始发生过拟合。
...
由于采用余弦衰减，如果 epoch 轮数过多，会导致学习率下降过慢，导致训练在前期发散，从而早停；如果不采用学习率衰减又会导致后续训练效果不佳，因此要合理控制 epoch 轮数，和初始的学习率值，当前实验中，初始学习率为0.003，轮数为 40 轮，这样可以保证模型得到充分训练。**同时这也是实验最耗时的部分。**
